...
/*   Updated: 2022/02/25 10:59:17 by vchevill         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (x_wind < WINDOW_WIDTH)
	{
		...
		while (y_wind < wall_size / 2 + offset_y)
		{
			...
			if (cub3d->map.ray_has_hit_x == 0  && ray_abs_angle > M_PI / 2 && ray_abs_angle < (3 * M_PI)/2)
			{
				...
				ft_pixel_texture_color(cub3d, cub3d->texture_e, 1 , y_wall / ratio);
			...
			else if (cub3d->map.ray_has_hit_x == 0)
			{
				...
				ft_pixel_texture_color(cub3d, cub3d->texture_w,  1, y_wall / ratio);
				put_pixel_to_image_3d(cub3d, x_wind, y_wind, create_trgb(0, cub3d->color_texture[0], cub3d->color_texture[1], cub3d->color_texture[2]));
			}
			else if (cub3d->map.ray_has_hit_x == 1  && ray_abs_angle > 0 && ray_abs_angle < M_PI)
			{
				...
				ft_pixel_texture_color(cub3d, cub3d->texture_n,  abs(ft_texture_x_offset(cub3d, ray_abs_angle, ray_length)), y_wall / ratio);
				put_pixel_to_image_3d(cub3d, x_wind, y_wind, create_trgb(0, cub3d->color_texture[0], cub3d->color_texture[1], cub3d->color_texture[2]));
			}
			else
			{
				ratio = wall_size / cub3d->texture_s.height;
				ft_pixel_texture_color(cub3d, cub3d->texture_s,  abs(ft_texture_x_offset(cub3d, ray_abs_angle, ray_length)), y_wall / ratio);
				put_pixel_to_image_3d(cub3d, x_wind, y_wind, create_trgb(0, cub3d->color_texture[0], cub3d->color_texture[1], cub3d->color_texture[2]));
			}
			y_wind++;
		}
