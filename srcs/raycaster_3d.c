...
/*   Updated: 2022/02/24 16:43:26 by vchevill         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (x < WINDOW_WIDTH)
	{
		...
		while (y < wall_size / 2 + offset_y)
		{
			...
			if (cub3d->map.ray_has_hit_x == 0  && angle_tmp > M_PI / 2 && angle_tmp < (3 * M_PI)/2)
			{
				...
				ft_pixel_texture_color(cub3d, cub3d->texture_e, (cub3d->ray_hit_dy % CUBE_SIZE), y_wall / ratio);
			...
			else if (cub3d->map.ray_has_hit_x == 0)
			{
				...
				ft_pixel_texture_color(cub3d, cub3d->texture_w, cub3d->ray_hit_dy % CUBE_SIZE , y_wall / ratio);
				put_pixel_to_image_3d(cub3d, column, y, create_trgb(0, cub3d->color_texture[0], cub3d->color_texture[1], cub3d->color_texture[2]));
			}
			else if (cub3d->map.ray_has_hit_x == 1  && angle_tmp > 0 && angle_tmp < M_PI)
			{
				ratio = wall_size / cub3d->texture_n.height;
				ft_pixel_texture_color(cub3d, cub3d->texture_n, cub3d->ray_hit_dx % CUBE_SIZE, y_wall / ratio);
				put_pixel_to_image_3d(cub3d, column, y, create_trgb(0, cub3d->color_texture[0], cub3d->color_texture[1], cub3d->color_texture[2]));
			}
			else
			{
				ratio = wall_size / cub3d->texture_s.height;
				ft_pixel_texture_color(cub3d, cub3d->texture_s, cub3d->ray_hit_dx % CUBE_SIZE, y_wall / ratio);
				put_pixel_to_image_3d(cub3d, column, y, create_trgb(0, cub3d->color_texture[0], cub3d->color_texture[1], cub3d->color_texture[2]));
			}
			y++;
		}
